    ...
    def has_one_attached_with(name, path:) # rubocop:disable Naming/PredicateName
      ...
      define_method "#{name}=" do |attachable|
        ...
        if self.class == OrderDocument
          custom_key = "#{Date.today}_#{self.document.name.gsub(' ', '_')}"
        elsif self.class == OrderAccount
          custom_key = "#{self.account.name}/#{Date.today}_Résiliation_#{self.account.name.gsub(' ', '_')}.#{self.resiliation_file.filename.extension}"
        end

class OrderAccount < ApplicationRecord
  ...
  def notify_resiliation_send
    Notification.create(
      content: "Demande de résiliation du contrat '#{self.account.name}' de #{self.order.decorate.deceased_full_name} envoyée",
      order: self.order
    )
  end
  ...
  def notify_resiliation_success
    Notification.create(
      content: "Contrat '#{self.account.name}' de #{self.order.decorate.deceased_full_name} résilié",
      order: self.order
    )
  end
